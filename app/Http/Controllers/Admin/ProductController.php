    ...
    public function manage_product_process(Request $request)
    {
        ...
        if($request->hasfile('image')){
            if($request->post('id')>0){                
                ...
                if(Storage::exists('/media'.$arrImage[0]->image)){
                    Storage::delete('/media'.$arrImage[0]->image);
                }
            }
            $image=$request->file('image');
            $ext=$image->extension();
            $image_name=time().'.'.$ext;
            $image->storeAs('/media',$image_name);
        ...
        foreach($skuArr as $key=>$val){
            ...
            if($request->hasFile("attr_image.$key")){
                ...
                $image_name=$rand.'.'.$ext;
                $request->file("attr_image.$key")->storeAs('/media',$image_name);
        ...
        foreach($piidArr as $key=>$val){
            ...
            if($request->hasFile("images.$key")){
                ...
                $image_name=$rand.'.'.$ext;
                $request->file("images.$key")->storeAs('/media',$image_name);
