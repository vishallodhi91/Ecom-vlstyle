...
class FrontController extends Controller
{
    ...
    public function category(Request $request, $slug)
    {
        ...
        $parant_id = Category::where('category_slug', $slug)->value('id');

        if ($parant_id < 7) {
            $query = $query->where(['categories.parent_category_id' => $parant_id]);
        } else {
    ...
    public function registration_process(Request $request)
    {
        ...
        if (!$valid->passes()) {
            ...
            if ($query) {
                ...
                return response()->json(['status' => 'success', 'msg' => "Registration successfully. Now you can login"]);
        ...
        if ($order_id > 0) {
            ...
            if ($request->payment_type == 'Gateway') {
                ...
                curl_setopt(
                    ...
                    CURLOPT_HTTPHEADER,
                    array(
                        "X-Api-Key:test_5813faf2377ae0e29cc3ad9ce76",
                        "X-Auth-Token:test_038d9a54ef8e594043089088fd3"
                    )
                );
                $payload = array(
                    'purpose' => 'Buy Product',
